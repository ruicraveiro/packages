...
class Camera
    ...
    /**
     * Sets new video stabilization mode from dart.
     *
     * @param result  Flutter result.
     * @param newMode New mode.
     */
    public void setVideoStabilizationMode(final Result result, @NonNull VideoStabilizationMode newMode) {
        final VideoStabilizationFeature videoStabilizationFeature = cameraFeatures.getVideoStabilization();
        videoStabilizationFeature.setValue(newMode);
        videoStabilizationFeature.updateBuilder(previewRequestBuilder);

        refreshPreviewCaptureSession(
                () -> result.success(null),
                (code, message) ->
                        result.error("setVideoStabilizationModeFailed", "Could not set video stabilization mode.", null));
    }
