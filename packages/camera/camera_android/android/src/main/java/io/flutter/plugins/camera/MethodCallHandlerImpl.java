    ...
    public void onMethodCall(@NonNull MethodCall call, @NonNull final Result result) {
        switch (call.method) {
            ...
            case "setVideoStabilizationMode": {
                String modeStr = call.argument("mode");
                VideoStabilizationMode mode = VideoStabilizationMode.getValueForString(modeStr);
                if (mode == null) {
                    result.error("setVideoStabilizationMode", "Unknown video stabilization mode " + modeStr, null);
                    return;
                }
                try {
                    camera.setVideoStabilizationMode(result, mode);
                } catch (Exception e) {
                    handleException(e, result);
                }
                break;
